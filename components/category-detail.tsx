...
export default function CategoryDetail({ category, bankingData, onBack, lastUpdated, isLoading = false }: CategoryDetailProps) {
  ...
  const metrics = [
    ...
    { label: "Day-to-Day", value: dtdDifference, format: true, colors: dtdColors, isDifference: true },
    { label: "Month-to-Date", value: mtdDifference, format: true, colors: mtdColors, isDifference: true },
    { label: "Year-to-Date", value: ytdDifference, format: true, colors: ytdColors, isDifference: true },
    { label: "Achievement %", value: achievement, format: false, colors: colors },
  ]
      ...
      <div className="flex-1 overflow-y-auto">
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 pb-8">
          {metrics.map((metric, index) => {
            ...
            return (
              <Card key={index} className={`p-6 border-2 ${metricColors.border} ${metricColors.bg}`}>
                <p className="text-sm text-muted-foreground mb-2">{metric.label}</p>
                <p className={`font-bold text-2xl ${metricColors.text}`}>
                  {metric.isDifference
                    ? formatDifference(metric.value as number)
                    : metric.format
        ...
        <div className="mt-8">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {data.segments.map((segment: any, index: number) => {
              const segmentAchievement = calculateAchievement(segment.value, segment.target, category)
              const segmentColors = getAchievementColor(segmentAchievement)

              return (
                <Card key={index} className={`p-6 border-2 ${segmentColors.border} ${segmentColors.bg}`}>
                  <h3 className="text-lg font-semibold text-primary mb-4">{segment.name}</h3>
                  <div className="space-y-3">
                    <div>
                      <p className="text-xs text-muted-foreground">Value</p>
                      <p className="text-xl font-bold text-foreground">{segment.value.toLocaleString()}</p>
                    </div>
                    <div>
                      <p className="text-xs text-muted-foreground">Target</p>
                      <p className="text-lg font-semibold text-foreground">{segment.target.toLocaleString()}</p>
                    </div>
                    <div className="pt-3 border-t-2 border-gray-200 dark:border-gray-700">
                      <div className="flex items-center justify-between">
                        <span className="text-xs font-medium text-muted-foreground">Achievement</span>
                        <span className={`text-sm font-bold ${segmentColors.text}`}>{segmentAchievement}%</span>
                      </div>
                    </div>
                  </div>
                </Card>
